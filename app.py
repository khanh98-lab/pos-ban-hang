...
# 1. Cấu hình giao diện & CSS Ẩn Header/Footer/Menu
...
# Hàm parse chuỗi có dấu chấm ngược lại thành số nguyên
...
# TRA CỨU MÃ SỐ THUẾ (Tối ưu fallback qua 4 nguồn)
def tra_cuu_mst_toi_uu(mst):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            h1_tag = soup.find('h1') or soup.find('div', class_='company-name')
            if h1_tag:
                ten_cty = h1_tag.get_text(strip=True)
                
            for div in soup.find_all(['div', 'li', 'p', 'tr']):
                txt = div.get_text()
                if 'Địa chỉ:' in txt or 'Địa chỉ' in txt:
                    dia_chi = txt.replace('Địa chỉ:', '').replace('Địa chỉ', '').strip()
                    break
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            h1_tag = soup.find('h1') or soup.find('div', class_='title-company')
            if h1_tag:
                ten_cty = h1_tag.get_text(strip=True)

            for tr in soup.find_all(['tr', 'p', 'div']):
                txt = tr.get_text()
                if 'Địa chỉ:' in txt or 'Địa chỉ' in txt:
                    dia_chi = txt.replace('Địa chỉ:', '').replace('Địa chỉ', '').strip()
                    break
...
# Load dữ liệu trực tiếp từ Google Sheets
@st.cache_data(ttl=5)
def load_data():
    return data_helper.load_all_sheets()
...
if not st.session_state["is_nv_logged_in"] and "nv_id" in query_params:
    ...
    if not df_nv.empty and 'ma_nv' in df_nv.columns:
        row_matched = df_nv[df_nv['ma_nv'] == logged_id]
        if not row_matched.empty:
            r_nv = row_matched.iloc[0]
            st.session_state["is_nv_logged_in"] = True
            st.session_state["logged_nv_info"] = {
                "ma_nv": logged_id,
                "ten_nv": r_nv.get('ten_nv', ''),
                "chi_nhanh": r_nv.get('chi_nhanh', ''),
                "nha_may": r_nv.get('nha_may', 'Tất cả')
            }
...
def cb_luu_khach_moi():
    ...
    data_helper.update_sheet("KhachHang", df_kh)
    st.cache_data.clear()
    ...
    st.session_state["msg_kh_success"] = f"Đã lưu thành công khách hàng: **{ten_hien_thi}** lên Google Sheets!"
...
with tab_pos:
    ...
    if not is_logged:
            ...
            if st.button("🔓 Đăng nhập phiên", use_container_width=True, type="primary"):
                if not df_nv_filtered.empty and ten_nv_chon in df_nv_filtered['ten_nv'].values:
                    ...
                    if pin_input.strip() == correct_pin:
                        ma_nv_log = str(row_nv.get('ma_nv', ''))
                        st.session_state["is_nv_logged_in"] = True
                        st.session_state["logged_nv_info"] = {
                            "ma_nv": ma_nv_log,
                            "ten_nv": ten_nv_chon,
        ...
        with col_logout:
            if st.button("🔒 Đăng xuất / Đổi NV", use_container_width=True):
                ...
                st.session_state["logged_nv_info"] = {}
                if "nv_id" in st.query_params:
                    del st.query_params["nv_id"]
                st.rerun()
    ...
    with tab_kh2:
        ...
        if btn_tracuu:
            if mst_input:
                with st.spinner("Đang tra cứu từ VietQR / Masothue / Thongtincongty / Thuvienphapluat..."):
    ...
    if st.session_state["gio_hang"]:
        ...
        if st.button("✅ BẤM LƯU ĐƠN HÀNG NÀY", use_container_width=True, type="primary", key="btn_luu_don_hang"):
            if not st.session_state["is_nv_logged_in"]:
                ...
                st.success("🎉 ĐÃ LƯU ĐƠN HÀNG THÀNH CÔNG LÊN GOOGLE SHEETS!")
                st.session_state["gio_hang"] = []
                st.cache_data.clear()
                st.rerun()
    else:
...
with tab_baocao:
    ...
    if is_admin:
        ...
        nv_info = st.session_state["logged_nv_info"]
        cn_bc = nv_info["chi_nhanh"]
        nv_bc = nv_info["ten_nv"]
        with col_bc2:
        ...
        cn_bc = "Tất cả"
        nv_bc = "Tất cả"
        st.warning("⚠️ Bạn cần Đăng nhập Nhân viên ở Tab 1 hoặc xác thực Admin ở Tab 4 (Mã 9999) để xem báo cáo!")
    ...
    if is_admin or is_logged:
        ...
        if not df_bc.empty:
            ...
            with st.expander(f"📄 **Danh sách {len(df_don_hang)} đơn phát sinh trong ngày (Bấm để xem/xóa)**", expanded=False):
                for idx, row in df_don_hang.iterrows():
                    ...
                    if is_admin:
                        ...
                        c_info, c_view = st.columns([7.5, 2.5])
            ...
            # ================= XỬ LÝ CHỈNH SỬA SỐ LƯỢNG & ĐƠN GIÁ DÀNH CHO ADMIN =================
            if is_admin:
                st.write("🔓 **Chế độ Admin (Code 9999): Cho phép chỉnh sửa SỐ LƯỢNG & ĐƠN GIÁ trực tiếp**")
                ...
                # TỰ ĐỘNG TÍNH LẠI THÀNH TIỀN NGAY LẬP TỨC CHO MỖI DÒNG KHI SỬA SL/GIÁ
...
# ==================== TAB 4: QUẢN LÝ ADMIN ====================
with tab_admin:
    st.subheader("⚙️ Cấu Hình Hệ Thống Professional POS (Đồng Bộ Google Sheets)")
    ...
    if not st.session_state["admin_unlocked"]:
        ...
        with st.expander("➕ **Thêm Nhân Viên Theo Chi Nhánh / Nhà Máy**"):
            ...
            if st.button("💾 Lưu Nhân Viên Mới", key="btn_save_new_nv"):
                if new_ten_nv and final_nm_nv:
                    ...
                    data_helper.update_sheet("NhanVien", df_nv)
                    st.cache_data.clear()
                    st.session_state["msg_admin_success"] = f"🎉 Đã thêm thành công nhân viên: **{new_ten_nv}** lên Google Sheets!"
                    st.rerun()
        ...
        with st.expander("➕ **Thêm Mặt Hàng / Loại Thuốc Mới**"):
            ...
            nm_vt_choice = st.selectbox("Thu thuộc Nhà Máy / Hãng:", options=[x for x in opts_nhamay_select if x != "Tất cả"], key="sb_add_vt_nm")
            ...
            if st.button("💾 Lưu Mặt Hàng Mới", key="btn_save_new_vt"):
                if new_ten_vt and final_nm_vt:
                    ...
                    data_helper.update_sheet("HangHoa", df_vt)
                    st.cache_data.clear()
                    st.session_state["msg_admin_success"] = f"🎉 Đã thêm thành công mặt hàng: **[{final_ma_vt}] {new_ten_vt}** lên Google Sheets!"
                    st.rerun()
        ...
        with st.expander("📁 **1. Quản Lý & Sửa Danh Sách Nhân Viên**"):
            if not df_nv.empty:
                ...
                if st.button("💾 Lưu Thay Đổi Nhân Viên", key="btn_save_nv_changes"):
                    ...
                    st.cache_data.clear()
                    st.session_state["msg_admin_success"] = "✅ Đã cập nhật danh sách nhân viên lên Google Sheets!"
                    st.rerun()
        ...
        with st.expander("📁 **2. Quản Lý & Sửa Danh Sách Mặt Hàng**"):
            if not df_vt.empty:
                ...
                if st.button("💾 Lưu Thay Đổi Mặt Hàng", key="btn_save_vt_changes"):
                    ...
                    st.cache_data.clear()
                    st.session_state["msg_admin_success"] = "✅ Đã cập nhật danh sách mặt hàng lên Google Sheets!"
                    st.rerun()
        ...
        with st.expander("📁 **3. Quản Lý & Sửa Danh Sách Khách Hàng**"):
            if not df_kh.empty:
                ...
                if st.button("💾 Lưu Thay Đổi Khách Hàng", key="btn_save_kh_changes"):
                    ...
                    st.cache_data.clear()
                    st.session_state["msg_admin_success"] = "✅ Đã cập nhật danh sách khách hàng lên Google Sheets!"
                    st.rerun()
